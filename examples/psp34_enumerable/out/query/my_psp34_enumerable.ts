...
import type {
	...
	Result,
} from "@artat9/typechain-types";
import type { QueryReturnType } from "@artat9/typechain-types";
import {
	queryJSON,
	queryOkJSON,
	handleReturnType,
...
//@ts-ignore
import { ReturnNumber } from "@artat9/typechain-types";
		...
		return queryJSON(
			...
			(result) => {
				return handleReturnType(
					result,
					getTypeDescription(1, DATA_TYPE_DESCRIPTIONS)
				);
			}
		);
	}
	...
	approve(
		...
		return queryOkJSON(
			...
			(result) => {
				return handleReturnType(
					result,
					getTypeDescription(23, DATA_TYPE_DESCRIPTIONS)
				);
			}
		);
	}
	...
	ownerOf(
		...
		return queryJSON(
			...
			(result) => {
				return handleReturnType(
					result,
					getTypeDescription(19, DATA_TYPE_DESCRIPTIONS)
				);
			}
		);
	}
	...
	tokenByIndex(
		...
		return queryOkJSON(
			...
			(result) => {
				return handleReturnType(
					result,
					getTypeDescription(26, DATA_TYPE_DESCRIPTIONS)
				);
			}
		);
	}
